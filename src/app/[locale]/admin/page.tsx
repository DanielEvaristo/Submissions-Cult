...
import {
  ...
  Activity, BarChart2, ShieldAlert, Music, UserCheck, X, Eye,
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "submissions" | "artists" | "claim_accounts">("overview");
  ...
  const [approvedResult, setApprovedResult] = useState<{ email: string; tempPassword: string } | null>(null);
  ...
  const loadClaimAccounts = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/admin/claim-accounts?limit=${claimLimit}&page=${claimPage}&search=${encodeURIComponent(claimSearch)}`);
      if (res.ok) {
        const data = await res.json();
    ...
    <div className="max-w-6xl mx-auto px-8 py-12 space-y-16 animate-reveal">
      ...
      <div className="flex flex-wrap">
        ...
        <button 
          ...
          className={`relative px-8 py-5 font-black text-xs uppercase tracking-[0.2em] border-t-4 border-x-4 border-white/10 transition-all ${activeTab === 'claim_accounts' ? 'bg-[#F5E000] text-black translate-y-1' : 'bg-black text-white opacity-20 hover:opacity-100'}`}
        >
          CLAIM ACCOUNTS
          {claimTotal > 0 && (
      ...
      {/* ── CLAIM ACCOUNTS TAB (outside stats block — loads independently) ── */}
      {activeTab === 'claim_accounts' && (
        <div className="space-y-10 animate-reveal">
          <SectionHeader icon={UserCheck} title="CLAIM_ACCOUNTS" subtitle="Cuentas importadas desde legacy esperando ser reclamadas" />
            ...
            <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-8">
              <div className="bg-black border-4 border-[#F5E000] p-10 max-w-md w-full space-y-6">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-black text-xs uppercase tracking-[0.3em] text-[#F5E000] mb-2">CUENTA ACTIVADA</p>
                    <p className="font-black text-2xl text-white">{approvedResult.email}</p>
                  </div>
                  <button onClick={() => setApprovedResult(null)} className="text-white/40 hover:text-white"><X size={24} /></button>
                </div>
                <div className="border-4 border-white/10 p-6 bg-white/5">
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2">CONTRASEÑA TEMPORAL</p>
                  <p className="font-black text-3xl text-[#F5E000] tracking-widest">{approvedResult.tempPassword}</p>
                </div>
                ...
                <button
                  onClick={() => { navigator.clipboard.writeText(approvedResult!.tempPassword); }}
          ...
          {claimLoading && claimAccounts.length === 0 ? (
            <div className="flex items-center gap-4 py-16 text-white">
              ...
              <span className="font-black uppercase tracking-widest text-sm">CARGANDO...</span>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">
                  {claimTotal} cuenta{claimTotal !== 1 ? 's' : ''} pendiente{claimTotal !== 1 ? 's' : ''}
                </p>
              ...
              {claimAccounts.length === 0 ? (
                <div className="border-4 border-white/10 p-16 text-center">
                  ...
                  <p className="font-black text-xl uppercase tracking-tighter text-white">SIN RESULTADOS</p>
                  <p className="text-xs font-bold uppercase tracking-widest text-white/30 mt-2">No se encontraron cuentas con esos filtros.</p>
                </div>
              ) : (
                <>
                      ...
                      >
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-3">
                            <span className="font-black text-lg text-white">{u.artistName || u.name || "—"}</span>
                            <span className="px-2 py-0.5 bg-[#F5E000]/20 text-[#F5E000] text-[9px] font-black uppercase tracking-widest">PENDING_CLAIM</span>
                          </div>
                          ...
                          <div className="flex gap-6 text-[9px] font-black uppercase tracking-widest text-white/30 mt-2">
                            ...
                            <span>{u._count?.submissions ?? 0} submission{(u._count?.submissions ?? 0) !== 1 ? 's' : ''}</span>
                        ...
                        <div className="flex gap-3 shrink-0">
                          ...
                          >
                            {processingId === u.id ? <Loader2 size={14} className="animate-spin" /> : <BadgeCheck size={14} />}
                            APROBAR
                          </button>
                          <button
                  ...
                  {claimTotal > claimLimit && (
                    <div className="flex items-center justify-between border-t-2 border-white/10 pt-6 mt-6">
                      <p className="text-[10px] font-black uppercase tracking-widest text-white/40">
                        MOSTRANDO {(claimPage - 1) * claimLimit + 1} - {Math.min(claimPage * claimLimit, claimTotal)} DE {claimTotal}
                      </p>
                      <div className="flex gap-2">
                        <button
                        ...
                        <button
                          ...
                          disabled={claimPage * claimLimit >= claimTotal || claimLoading}
                          className="px-6 py-3 border-2 border-white/10 text-white font-black text-[10px] uppercase tracking-widest hover:border-[#F5E000] hover:text-[#F5E000] disabled:opacity-30 disabled:hover:border-white/10 disabled:hover:text-white transition-colors"
                        >
                          SIGUIENTE →
                        </button>
