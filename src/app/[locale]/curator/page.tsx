...
export default function CuratorDashboard() {
  ...
  return (
    <div className="h-[calc(100vh-64px)] lg:h-auto flex overflow-hidden">
      
      {/* ── Left Column: Inbox + History Tabs ── */}
      <div className={`w-full lg:w-1/3 lg:min-w-[350px] border-r-4 border-white/10 bg-black flex-col h-full ${selectedId ? 'hidden lg:flex' : 'flex'}`}>
        {/* Header */}
      ...
      <div className={`flex-1 bg-bg flex-col h-full overflow-y-auto ${selectedId ? 'flex' : 'hidden lg:flex'}`}>
        {!selectedSub ? (
          ...
          <>
            ...
            <button
              ...
              className="lg:hidden sticky top-0 z-10 bg-black text-[#F5E000] p-4 flex items-center gap-3 font-sans font-black text-xs uppercase tracking-widest border-b-2 border-[#F5E000]/20"
            >
              <ChevronLeft size={16} strokeWidth={3} /> BACK TO INBOX
            </button>
            <div className="max-w-4xl mx-auto w-full p-4 sm:p-8 animate-fade-in space-y-8">
              {/* Header / Track Info */}
              <div className="flex flex-col md:flex-row gap-8 items-start border-b-4 border-white/10 pb-10">
                <div className="w-32 h-32 sm:w-48 sm:h-48 shrink-0 bg-black border-4 border-white/20 overflow-hidden flex items-center justify-center shadow-[8px_8px_0px_0px_rgba(245,224,0,0.1)]">
                ...
                <div className="flex-1 pt-2">
                  ...
                  <h2 className="font-sans text-4xl sm:text-6xl font-black text-white tracking-tighter leading-[0.9] uppercase mb-4">
                    {selectedSub.trackTitle}
                  </h2>
                  <p className="font-sans text-xl sm:text-2xl text-white font-black uppercase tracking-tight opacity-40">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <section>
                    ...
                    {selectedSub.status === "PENDING" ? (
                      <div className="p-12 border-4 border-white/10 bg-black text-center">
                        <p className="font-sans text-xs font-black uppercase tracking-widest text-white/20 mb-4">
                          This submission has not been claimed yet.
                        </p>
                        <button
                          onClick={() => handleAction("claim")}
                          disabled={actionLoading === "claim"}
                          className="px-8 py-4 bg-[#F5E000] text-black font-sans font-black text-xs uppercase tracking-[0.2em] hover:bg-white transition-all flex items-center justify-center gap-3 mx-auto"
                        >
                          {actionLoading === "claim" ? <Loader2 className="animate-spin" size={16}/> : <>CLAIM & REVIEW</>}
                        </button>
                      </div>
                    ) : selectedSub.curatorId !== session?.user?.id ? (
                      <div className="p-12 border-4 border-white/10 bg-black text-center">
                      ...
                      <div className="p-6 sm:p-10 border-4 border-white/10 bg-black space-y-10 shadow-[12px_12px_0px_0px_rgba(245,224,0,0.05)]">
                ...
                <aside className="space-y-8">
                  <section>
                    ...
                    <div className="p-6 border border-border bg-bg-surface rounded-xl space-y-3">
                      <InfoRow label="Location" value={selectedSub.user.city ? `${selectedSub.user.city}, ${selectedSub.user.country}` : selectedSub.user.country} />
                      <InfoRow label="Age" value={selectedSub.user.ageRange?.replace(/_/g, " ")} />
                      <InfoRow label="Languages" value={selectedSub.user.musicLanguages?.join(", ")} />
                      <InfoRow label="Role" value={selectedSub.user.roleType?.replace(/_/g, " ")} />
                  ...
                  <section>
                    ...
                    <div className="p-6 border border-border bg-bg-surface rounded-xl space-y-3">
                      <SocialLink platform="Spotify" url={selectedSub.user.spotifyUrl} />
                      <SocialLink platform="Instagram" url={selectedSub.user.instagram} />
                      <SocialLink platform="TikTok" url={selectedSub.user.tiktok} />
                      <SocialLink platform="YouTube" url={selectedSub.user.youtube} />
                      <SocialLink platform="SoundCloud" url={selectedSub.user.soundcloudUrl} />
                      <SocialLink platform="Website" url={selectedSub.user.website} />
...
function buildUrl(label: string, url: string): string | null {
  const val = url?.trim() ?? "";
...
function SocialLink({ platform, url }: { platform: string; url?: string | null }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="flex items-center gap-2 p-3 transition-all bg-black border-2 border-transparent rounded-lg hover:border-[#F5E000] group"
    >
      <span className="font-sans text-sm font-black uppercase tracking-wider text-white group-hover:text-[#F5E000]">
        {platform}
      </span>
      <ExternalLink size={14} className="text-white group-hover:text-[#F5E000]" />
    </a>
  );
}
